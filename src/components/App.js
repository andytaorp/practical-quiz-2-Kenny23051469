...
function App() {
  ...
  const handleAddHabit = (habitName) => {
    ...
    setHabits((prevHabit) => [...prevHabit, habitName]);
  };

  const handleToggleHabit = (id) => {
    // TODO: write code to toggle a habit's status
    setHabits((prevHabits) =>
      prevHabits.map((habit) => (habit.id === id ? { ...habit, completed: !habit.completed } : habit))
    );
  };

  const handleDeleteHabit = (id) => {
    // TODO: write code to delete a habit
    setHabits((prevHabit) => prevHabit.filter((habit) => habit.id !== id));
  };
